...
class HTML_groupjiveCategoryGroups {
	...
	static function showGroups( $rows, $pageNav, $category, $user, $plugin ) {
		...
		$categoryGroupsDescLimit	=	(int) $plugin->params->get( 'category_groups_desc_limit', 150 );
		...
		if ( $rows ) foreach ( $rows as $row ) {
			...
			$return					.=					'</div>'
									.				'</div>'
									.				'<div class="gjContentBodyInfo">' . ( $row->getDescription( $categoryGroupsDescLimit ) ? '<div class="well well-small">' . $row->getDescription( $categoryGroupsDescLimit ) . '</div>' : null ) . '</div>'
