...
class HTML_groupjiveTabGroups {
	...
	static function showGroups( $rows, $pageNav, $displayed, $user, $plugin, $tabbed ) {
		...
		$groupTabDescLimit	=	(int) $plugin->params->get( 'group_tab_desc_limit', 150 );
		...
		if ( $rows ) foreach ( $rows as $row ) {
			...
			$return			.=						'</div>'
							.					'</div>'
							.					'<div class="gjContentBodyInfo">' . ( $row->getDescription( $groupTabDescLimit ) ? '<div class="well well-small">' . $row->getDescription( $groupTabDescLimit ) . '</div>' : null ) . '</div>'
