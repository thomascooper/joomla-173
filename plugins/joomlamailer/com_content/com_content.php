    ...
    */
    private function renderTable ($items) {
        ...
        foreach ($items as $item) {
            ...
            $table .= '<td>';
            $table .= '<label for="' . $this->id . '_full_' . $item->id . '_0">' . JText::_('Intro') . '</label>';
            $table .= '<input type="radio" name="' . $this->id . '_full_' . $item->id . '" id="' . $this->id . '_full_' . $item->id . '_0" value="0" checked="checked">';
            $table .= '<br />';
            $table .= '<label for="' . $this->id . '_full_' . $item->id . '_1">' . JText::_('Full') . '</label>';
        ...
        if (isset($repeater[0])) {
            ...
            if (count($componentsPostData)) {
                foreach ($componentsPostData as $v) {
                    ...
                    $query = $this->db->getQuery(true);
                    $query->select($this->db->qn('title') . ',' . $this->db->qn('introtext') . ',' . $this->db->qn('fulltext'))
                        ->from('#__content')
