...
class plgjsnimageshowSourceFlickrInstallerScript
{
	...
	private function _updateSchema()
	{
		...
		$eid = $row->find(array('element' => 'sourceflickr', 'type' => 'plugin'));
		if ($eid)
		{
			$db = JFactory::getDBO();
	...
	private function _getInfo($id)
	{
		...
		$query->where('element=\'sourceflickr\' AND type=\'plugin\' AND folder=\'jsnimageshow\' AND extension_id = ' . $id);
		$db->setQuery($query);
