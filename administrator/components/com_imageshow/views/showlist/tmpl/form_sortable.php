...
<div class="jsn-showlist-media" id="jsn-showlist-media-layout">
	<div class="ui-layout-west" id="panel-west">
		<div class="source-media-header"><h3><?php echo JText::_('SHOWLIST_SHOWLIST_SOURCE_IMAGES'); ?></h3></div>
		<div class="source-media-panel-container">
			<div class="panel-right jsn-bootstrap">
				<div class="panel-header clearafter jsn-iconbar" id="source-media-header">
					<a href="javascript: void(0);" class="media-show-grid pull-left" title="<?php echo JText::_('SHOWLIST_IMAGE_MANAGER_SELECT_THUMB_PRESENTATION_MODE'); ?>">
						<i class="jsn-icon16 jsn-icon-layout jsn-icon-thumbnails disabled"></i>
					</a>
					<a href="javascript: void(0);" class="media-show-list pull-left" title="<?php echo JText::_('SHOWLIST_IMAGE_MANAGER_SELECT_DETAILS_PRESENTATION_MODE'); ?>">
						<i class="jsn-icon16 jsn-icon-layout jsn-icon-thumbnaildetails disabled"></i>
					</a>
					<a href="javascript: void(0);" class="pull-right" id="move-selected-media-source" title="<?php echo JText::_('SHOWLIST_IMAGE_MANAGER_ADD_SELECTED_IMAGE'); ?>">
						<i class="jsn-icon16 jsn-icon-ok disabled"></i>
					</a>
				</div>
				<div class="source-media-container" id="source-media-container">
					<div class="source-items showgrid" id="source-items">
						...
						<div id="show-more-items">
							<button id="show-more-items-btn" class="btn"><?php echo JText::_('SHOWLIST_IMAGE_LOAD_MORE_IMAGES'); ?></button>
	...
	<div class="ui-layout-center" id="panel-center">
		<div class="source-media-header"><h3><?php echo JText::_('SHOWLIST_SHOWLIST_S_IMAGES');?></h3></div>
		<div class="source-media-panel-container jsn-bootstrap">
			<div class="panel-header clearafter jsn-iconbar" id="showlist-media-header">
				<a href="javascript: void(0);" class="media-show-grid pull-left" title="<?php echo JText::_('SHOWLIST_IMAGE_MANAGER_SELECT_THUMB_PRESENTATION_MODE'); ?>">
					<i class="jsn-icon16 jsn-icon-layout jsn-icon-thumbnails disabled"></i>
				</a>
				<a href="javascript: void(0);" class="media-show-list pull-left" title="<?php echo JText::_('SHOWLIST_IMAGE_MANAGER_SELECT_DETAILS_PRESENTATION_MODE'); ?>">
				...
				<a href="javascript: void(0);" class="pull-right" id="delete-media-showlist"  <?php if ($sync=='sync'){ echo 'style="display:none;"';}?> title="<?php echo JText::_('SHOWLIST_IMAGE_MANAGER_DELETE_SELECTED_IMAGE'); ?>">
					<i class="jsn-icon16 jsn-icon-trash disabled"></i>
				</a>
				<a href="javascript: void(0);" class="pull-right" id="edit-media-showlist"  <?php if ($sync=='sync'){ echo 'style="display:none;"';}?> title="<?php echo JText::_('SHOWLIST_IMAGE_MANAGER_EDIT_SELECTED_IMAGE'); ?>">
					<i class="jsn-icon16 jsn-icon-pencil disabled"></i>
				</a>
				<div class="clearbreak"></div>
			</div>
			<div class="source-media-container" id="showlist-media-container">
				<div class="showlist-items showgrid <?php if (!count($imagesStored)){ echo 'jsn-section-empty'; } ?>" id="showlist-items">
				...
				if ($countImagesStored)
				{
					foreach ($imagesStored as $image)
					{
						...
						?>
							<div class="media-item" id="<?php echo md5($image['image_extid']); ?>">
								<input class="item_id" type="hidden" value="<?php echo urlencode($image['image_extid']); ?>" />
								<input type="hidden" value="<?php echo $image['original_link']; ?>" id="linkcheck" name="linkcheck" />
								<input class="item_extid" type="hidden" value="<?php echo urlencode($image['album_extid']); ?>">
								<input class="item_detail" type="hidden" value="<?php echo htmlspecialchars(json_encode($processedImage), ENT_COMPAT, 'UTF-8'); ?>" />
								...
								<div class="item-info">
									<p><strong><?php echo $objJSNUtils->wordLimiter(strip_tags($image['image_title']), 20);?></strong></p>
									<p><?php echo $objJSNUtils->wordLimiter(strip_tags($image['image_description']), 30);?></p>
				...
				{
					?>
						<div class="jsn-bglabel <?php echo (trim($selectMode) == 'sync')?'showlist-sync-item-notice':'showlist-drag-drop-item-notice'?>">
							<?php
								if (trim($selectMode) == 'sync')
								{
							?>
...
<input type="hidden" value="" id="start_item_showlist" name="start" />
<input type="hidden" value="" id="stop_item_showlist" name="stop" />
</div>