...
<div id="jsn-showcase-install-themes-verify">
	<h3 class="jsn-section-header"><?php echo JText::_('SHOWCASE_THEME_CUSTOMER_VERIFICATION');?></h3>
	<?php
		if (isset($data))
		{
	...
	?>
	<p id="jsn-is-login-message"><?php echo JText::sprintf('SHOWCASE_THEME_LOGIN_DID_YOU_PURCHASE_IT', @$data->name)?></p>
				...
				<tr>
					<td colspan="2" align="center">
						<div class="form-actions">
							<button onclick="submitLoginForm(); return false;" class="link-button" type="button" id="submit-customer-verification"><?php echo (($this->canAutoDownload) ? JText::_('INSTALLER_INSTALL') : JText::_('INSTALLER_DOWNLOAD')); ?></button>
						</div>
					</td>
				</tr>
			</table>
...
<div id="jsn-showcase-manual-install" class="jsn-manual-install-form">
	<form method="post" enctype="multipart/form-data" action="index.php?option=com_imageshow&controller=installer&task=installThemeManual">
		<h1><?php echo JText::_('SHOWCASE_PROFILE_MANUAL_INSTALL_THEME')?></h1>
		<div><?php echo JText::sprintf('SHOWCASE_PROFILE_MANUAL_INSTALL_PLEASE_DOWNLOAD', @$data->name)?></div>
		...
		<input type="hidden" name="redirect_link" value="index.php?option=com_imageshow&controller=showcase&task=authenticate&layout=form_login&install_manual_redirect=1"/>
		<div class="form-actions">
