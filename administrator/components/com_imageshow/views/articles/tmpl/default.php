...
<div class="jsn-page-list">
	<div class="jsn-bootstrap">
		  <form class="form-inline" action="<?php echo JRoute::_('index.php?option=com_imageshow&controller=articles&tmpl=component&function=' . $function . '&' . JSession::getFormToken() . '=1'); ?>" method="post" name="adminForm" id="adminForm">
				...
				<table class="table table-bordered table-striped">
					 ...
					 <tbody>
						  <?php foreach ($this->items as $i => $item) : ?>
	 						  <tr onclick="if (window.parent) window.parent.<?php echo $this->escape($function); ?>('<?php echo $item->id; ?>', '<?php echo $this->escape(addslashes($item->title)); ?>', '<?php echo $this->escape($item->catid); ?>', null, '<?php echo $this->escape(ContentHelperRoute::getArticleRoute($item->id)); ?>');" >
	 								<td>
				...
				<div>
					 ...
					 <input type="hidden" name="return" value="index.php?option=com_imageshow&controller=image&task=linkpopup&tmpl=component&tab=article" />
