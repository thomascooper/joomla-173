...
class joomailermailchimpintegrationControllerInstaller extends joomailermailchimpintegrationController {
    ...
    private $dbQueries = array(
        ...
        array(
            'task' => 'create',
            'data' => "CREATE TABLE IF NOT EXISTS `#__joomailermailchimpsignup` (
                `id` int(11) unsigned NOT NULL AUTO_INCREMENT,
                `fname` varchar(100),
                `lname` varchar(100),
    ...
    private $extensions = array(
        'com_joomailermailchimpsignup',
        'plg_joomailermailchimpsignup',
        'mod_mailchimpsignup',
        'mod_mailchimpstats',
        ...
        'plg_joomlamailer_CommunityBuilder'
    );
    ...
    public function installext() {
        ...
        if (strpos($extension, 'plg_joomlamailer_CommunityBuilder') !== false) {
            if (!JFile::exists(JPATH_ADMINISTRATOR . '/components/com_comprofiler/comprofiler.xml')) {
                ...
                $res = array(
                    'label' => 'Incompatible',
                    'notification' => 'Please upgrade Community Builder and then install joomlamailer again.'
                );
            } else {
                ...
                if (!$cbInstallerPlugin->upload($packagesPath . $extension)) {
                    if ($cbInstallerPlugin->unpackDir()) {
                        $cbInstallerPlugin->cleanupInstall($packagesPath . $extension, $cbInstallerPlugin->unpackDir());
                    }
                    $res = array(
                        'error' => 'Unpacking failed.'
                    );
                } else if ($cbInstallerPlugin->install()) {
                    $cbInstallerPlugin->cleanupInstall($packagesPath . $extension, $cbInstallerPlugin->unpackDir());

                    $query = $db->getQuery(true)
                        ->update($db->qn('#__comprofiler_plugin'))
                    ...
                    $cbInstallerPlugin->cleanupInstall($packagesPath . $extension, $cbInstallerPlugin->unpackDir());
                    $res = array(
                        'error' => $cbInstallerPlugin->getError()
                    );
                }
            }
        } else {
            ...
            if (!$installer->install($package['dir'])) {
                ...
                if (strpos($extension, 'com_joomailermailchimpsignup') !== false) {
                    // remove signup component from backend menu
                    $query = $db->getQuery(true)
                        ->delete($db->qn('#__menu'))
                        ->where($db->qn('title') . ' = ' . $db->q('com_joomailermailchimpsignup'));
                    $db->setQuery($query)->execute();
                } else if (strpos($extension, 'mod_mailchimpstats') !== false) {
    ...
    private function updateColumns() {
        ...
        $res = $this->AddColumnIfNotExists('#__joomailermailchimpsignup', 'merges',"text NOT NULL", '');
        if ($res !== true) {
            $errors[] = $res;
        }
