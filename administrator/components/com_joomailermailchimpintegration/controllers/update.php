...
class joomailermailchimpintegrationControllerUpdate extends joomailermailchimpintegrationController {
    ...
    public function update() {
        ...
        if (!$updates->update_available) {
            $this->app->enqueueMessage('JM_ERR_UPDATE_NOUPDATES', 'error');
        ...
        $result = $this->getModel('update')->downloadPackage($package, $target);

        if ($result === false) {
            $this->app->enqueueMessage('JM_ERR_UPDATE_CANTDOWNLOAD', 'error');
            $this->app->redirect(JURI::base() . 'index.php?option=com_joomailermailchimpintegration');
        }
        ...
        if ($result === false) {
            $this->app->enqueueMessage('JM_ERR_UPDATE_CANTEXTRACT', 'error');
            $this->app->redirect(JURI::base() . 'index.php?option=com_joomailermailchimpintegration');
        }
